...
func PopulateContract(w http.ResponseWriter, r *http.Request) {
    ...
    //userAddr = common.HexToAddress("0xde9319abc7958e1939b472a811bab05f2d79f5fc")
    if len(r.MultipartForm.Value["UserAddr"])!=0 {
        userAddrStr := r.MultipartForm.Value["UserAddr"][0]
        if (common.IsHexAddress(userAddrStr) == false) {
            http.Error(w, GeneralError{"User contract address is incorrect"}.Error(),
                http.StatusInternalServerError)
            return
        }
    ...
    dataCert, err := GenerateCert(contrAddr, parentAddr)
    if err != nil {
    ...
    resStr += "Contract Addr = "+contrAddrStr+"\n"
    resStr += "Parent Addr = "+common.ToHex(parentAddr.Bytes())+"\n"
    if (gConfig.WebMode!=0){
        var formParam CSimpleFormParam
        ...
        formParam.Params = append(formParam.Params,
            CParam{"UserAddr",common.ToHex(userAddr.Bytes())})
...
func ValidateCert(w http.ResponseWriter, r *http.Request){
    ...
    isCertOK, revokeDate, jsonCertTree, iter, err := CheckCertTree(contrAddr, certHash)
    if(err != nil){
...
func GenerateCert(contrAddr common.Address, parentAddr common.Address) ([]byte, error){

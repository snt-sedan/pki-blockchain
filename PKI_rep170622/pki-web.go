...
func DownloadCaCert(w http.ResponseWriter, r *http.Request){
    ...
    http.ServeContent(w, r, "ca.crt", time.Now(), bytes.NewReader(certData))
}
...
func ConfirmHashCAData(currAddr common.Address, hash uint32,
    ...
    numRD, err := currContract.NumRegData(callOpts)
    if err!=nil {
        return false, time.Time{}, common.Address{}, 0, nil, err
    }
    ...
    for i := int64(0); i < numRD.Int64(); i++ {
        bi := big.NewInt(i)
        regDatum, err := currContract.RegData(callOpts, bi)
        if err != nil {
            return false, time.Time{}, common.Address{}, 0, nil,
                    GeneralError{fmt.Sprintf("Failed to get hash data: ", err)}
        }
        delRegDatum, err := currContract.DeletedRegData(callOpts, bi)
        if err != nil {
            return false, time.Time{}, common.Address{}, 0, nil,
                    GeneralError{fmt.Sprintf("Failed to get deleted data: ", err)}
        }
        blchHash, err := strconv.Atoi(string(regDatum.DataHash))
        if err != nil {
            return false, time.Time{}, common.Address{}, 0, nil,
        ...
        if(uint32(blchHash) == hash){
            if delRegDatum.DeletionDate.Int64() != 0 {
                retRevokeDate = time.Unix(delRegDatum.DeletionDate.Int64(), 0)
            }else{
                retIsHashFound = true
            }
            break

...
func ConfirmHashCAData(currAddr common.Address, hash []byte,
	...
	if err != nil {
		return false, time.Time{}, common.Address{}, nil, nil, err
	}
	...
	if contrAddr != currAddr {
		return false, time.Time{}, common.Address{}, nil, nil,
	...
	for i := int64(0); i < numRD.Int64(); i++ {
		...
		if CompBytes(hash, regDataHash[:]) == true {
...
/* return true if b1 equals b2, otherwise false */
func CompBytes(b1 []byte, b2 []byte) bool {
	if len(b1) != len(b2) {
		return false
	}
	for i := 0; i < len(b1); i++ {

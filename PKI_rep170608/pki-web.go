...
func PopulateContract(w http.ResponseWriter, r *http.Request) {
    ...
    for i, _ := range files {
        ...
        err = UpdateCertificate(file, contrAddr);
        if err != nil {
            fmt.Printf("Unable to update certificate: %v\n", err.Error())
            return
        }
        ...
        _, err = session.PopulateCertificate(dst4hash)
        if err != nil {
            log.Fatalf("Failed to populate blockchain: %v. ", err)
            fmt.Fprintln(w, err)
        }

        if (gConfig.WebMode!=0){
            var formParam CSimpleFormParam
            formParam.Param = common.ToHex([]byte(hashSum))
            formParam.Result = "Contract was successfully created"
            formParam.CallForm = "enroll-user"
...
func CreateContract(w http.ResponseWriter, r *http.Request){
    ...
    */
    ...
    trOpts.GasLimit = big.NewInt(2000000)
    contrAddr, _, /*contr*/_, err := DeployLuxUni_PKI(auth, gClient, common.Address{})
    /*
    ...
    */
    ...
    if (gConfig.WebMode!=0) {
        var formParam CSimpleFormParam
        formParam.Param = common.ToHex([]byte(contrAddr))
        formParam.Result = "Contract was successfully created"

...
contract LuxUni_PKI is owned {
    ...
    /*
        _ind is uint (uint256 which is 32 bytes) which consists of the following
            () - 12 senior bytes contain 12 first (senior) bytes of _contrAddr
            () - 20 junior bytes contain 20 first (senior) bytes of _dataHash
        Decoding is realised in EncodeEventInd 
    /
    function EncodeEventInd(bytes32 _dataHash, address _contrAddr) constant returns(uint _ind) {
        uint _intAddr = bytes(_contrAddr)
        for( uint _i=0; _i<20; _i++ ) {
            _ind <<= 8;
            _ind = _ind | uint(_dataHash[_i]);
        }
        return _ind;
    }*/
    ...
    */
    function newRegDatum(bytes32 _dataHash, bytes _algoHashID) onlyOwner returns(uint _regID) {
        ...
        reg.creationDate = now;
        numRegData = _regID+1;
        bytes memory bPtr = new bytes(_dataHash.length);
        uint i;
        for(i=0; i<_dataHash.length; i++) {
            bPtr[i] = _dataHash[i];
        }
        evLuxUni_NewRegDatumReturn(bytestr_to_uint(bPtr/*bytes(_dataHash)*/), _regID);
    }

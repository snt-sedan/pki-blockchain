...
/* uint256 -- 32 bytes
   ...
   uint256 in mapping below : 
        first 20 bytes -- address of smart contract holding the hashes in regData arrays
...
*/    
    ...
    function getEthAccCA(address _addrContr, uint96 _index) constant returns( address ) { 
    ...
    function newRegDatum(uint _regID, address _ethAccCA, address _contrAddr, string _fileName,
            string _description) onlyOwner returns(uint err) {
    ...
    /*       
        returns:
          ...
          result:
            ...
            -1 - error in parsing
            -2 - CA addr in the certificate does not correspond to _addrCA
            -3 - empty cert received for this CA
            -4 - empty addrCA parsed in this CA cert
    */
    function CheckCertForCA( bytes32 _hash ) constant returns(int _result, address _addrParent, bytes32 _newHash) {
        uint i;
        ...
        if( _errCode != 0 ) { return (-1, 0, ""); }
        if( _crtAddrCA == 0 ) { return (-4, 0, ""); }
...
contract LuxUni_validateCert {
    ...
    /*
        ...
        returns:
            10+n - OK, where n is the number of iteraction
            1  - certificate not found
            2  - certificate revoked
            -1 - error in parsing
            -2 - CA addr in the certificate does not correspond to _addrCA
            -3 - empty cert received for this CA
            -4 - empty addrCA parsed in this CA cert
            -5 - parent addr is null, but CA addr does not correspond to Root addr  
            -6 - too many iterations: limit (100?) is exceeded
    */
    function CheckCert( bytes32 _newHash, address _addrCA,
             address _addrRoot) constant returns(int _result) {
        int i;
        int _resCheck;
        address _addrParent;

...
func ValidateCert(w http.ResponseWriter, r *http.Request) {
	...
	var parentAddr common.Address = common.Address{}
	//var formHash common.Hash = common.Hash{}
	var isNoUpload bool = false
	...
	if cerr.errCode != 0 {
		if cerr.errCode == 1 {
			...
			/*hashInt := big.NewInt(0)
			hashInt, _ = hashInt.SetString(hashArr[0], 16)
			if hashInt == nil {
	...
	revokeDateStr := "No"
	/*zeroTime, err := time.Parse(response.RevokeDate.String(), time.Time{}.String())
	  if err != nil {
	      http.Error(w, fmt.Sprintf("ValidateCert: error in creation zero time: %v", err.Error()),
	          http.StatusInternalServerError)
	      return
	  }*/
	...
	if gConfig.WebMode != 0 {
		...
		formParam.Result = "Results of certificate check (certHash=" +
			fmt.Sprintf("%x", certHash) + ") for CA " // + contrAddr.String()
